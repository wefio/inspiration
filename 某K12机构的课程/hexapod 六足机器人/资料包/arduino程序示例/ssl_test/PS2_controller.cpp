...
#define TRANSLATEMODE     1       //平移模式
#define ROTATEMODE        2       //rotate旋转模式
#define SINGLELEGMODE     3       //单腿模式
#define GPPLAYERMODE      4


#define cTravelDeadZone 4      //The deadzone for the analog input from the remote —— 远程模拟输入死区
#define MAXPS2ERRORCNT  5     // How many times through the loop will we go before shutting off robot?——在关闭机器人之前，我们要经历多少次循环？

#define InitPos     30      //开机时站姿
#define OffPos      0       //关机时站姿
//=============================================================================
// Global - Local to this file only...
//=============================================================================
PS2X ps2x; // create PS2 Controller Class//创造PS2类
...
void InputController::Init(void)
{
  ...
  g_BodyYOffset = InitPos;  // 0 - Devon wanted...
  g_BodyYShift = 0;
...
void InputController::ControlInput(void)
{
  ...
  if ((ps2x.Analog(1) & 0xf0) == 0x70) {
    ...
    if (g_InControlState.fHexOn) {
      ...
      if (ps2x.ButtonPressed(PSB_PAD_RIGHT)) { // D-Right - Button Test
        if (g_InControlState.SpeedControl > 0) {
          ...
          MSound(SOUND_PIN, 1, 50, 2000);  //sound SOUND_PIN, [50\4000]
          ////Serial.println(g_InControlState.SpeedControl);
        }
      }
    ...
    // We may have lost the PS2... See what we can do to recover... -我们可能失去了PS2。。。看看我们能做些什么来恢复。。
